defineCustomElement(
  ...
  class TH extends H {
    ...
    show = (delta) => {
      ...
      const targettedPart = this.parts[delta === -1 ? this.currentPart + 1 : this.currentPart];
      ...
      if (this.replace) {
        this.parts[0].setAttribute("aria-hidden", delta === 1 ? "true" : "false");
        this.parts[1].setAttribute("aria-hidden", delta === -1 ? "true" : "false");
      } else if (targettedPart) {
        targettedPart.setAttribute("aria-hidden", delta === -1 ? "true" : "false");
      }
    };
    ...
    init = () => {
      ...
      this.parts.slice(1, this.parts.length).forEach((part) => part.setAttribute("aria-hidden", "true"));
      ...
      this.replace = this.getAttribute("data-replace") === "true";
    };
  }
);

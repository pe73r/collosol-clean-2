...
const getCart = async () => {
  return (await fetch(`${cartUrl()}.js`)).json();
};
...
defineCustomElement(
  ...
  class TailGift extends HTMLElement {
    ...
    clickDebouncer = debounc(() => this.onClick(), 50);
    i = () => {
      // const observerContainer = new MutationObserver(() => {
      console.log("observerContainer");
      this.input = this.querySelector('input[name="name"]');
      this.textarea = this.querySelector("textarea");
      this.label = this.querySelector("button");
      if (this.input && this.textarea && this.label && !this.isInit) {
        this.isInit = true;
    ...
    init = (inputs) => {
      console.log({ inputs });
      this.label.addEventListener("pointerdown", this.clickDebouncer);
    ...
    addKdoToCart = async (add) => {
      const res = await fetch(`${cartUrl()}/update.js`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json"
      ...
      this.label.setAttribute("aria-selected", String(add));
